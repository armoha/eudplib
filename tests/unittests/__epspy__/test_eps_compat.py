...
# (Line 3) EUDOnStart(function () { x += x; });
@EUDFunc
def _lambda1():
    x.__iadd__(x)

EUDOnStart(_lambda1)
# (Line 4) function test_compatibility() {
@EUDFunc
def f_test_compatibility():
    # (Line 5) static var ret = 0;
    ret = EUDVariable(0)
    # (Line 6) const empty = Db(i2b4(0));
    empty = Db(i2b4(0))
    # (Line 7) const cond = Forward();
    cond = Forward()
    # (Line 9) py_exec("from helper import *\n\
    # (Line 18) ");
    ...
    # (Line 20) cond.__lshift__(Memory(empty, AtLeast, 1));
    cond.__lshift__(Memory(empty, AtLeast, 1))
    # (Line 21) if (cond) { ret += 1 << 3; }
    if EUDIf()(cond):
